<script lang="ts">
  ...
  import { mainnetChain, wyzth_zkevmChain } from '../chain/chains';
...
<div class="dropdown dropdown-end mr-4">
  ...
  <ul
    ...
    <li>
      <button
        class="flex items-center px-2 py-4 hover:bg-dark-5 rounded-sm"
        on:click={() => switchChains(wyzth_zkevmChain)}>
        <svelte:component this={wyzth_zkevmChain.icon} height={24} />
        <span class="pl-1.5 text-left flex-1">{wyzth_zkevmChain.name}</span>
      </button>
    </li>
  </ul>
</div>
