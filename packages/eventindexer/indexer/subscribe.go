...
func (svc *Service) subscribeBlockProven(ctx context.Context, chainID *big.Int, errChan chan error) {
	sink := make(chan *wyzth_zkevml1.WYzth_zkevmL1BlockProven)

	sub := event.ResubscribeErr(svc.subscriptionBackoff, func(ctx context.Context, err error) (event.Subscription, error) {
		if err != nil {
			log.Errorf("svc.wyzth_zkevmL1.WatchBlockProven: %v", err)
		}
		log.Info("resubscribing to BlockProven events")

		return svc.wyzth_zkevml1.WatchBlockProven(&bind.WatchOpts{
			Context: ctx,
		}, sink, nil)
	})
...
func (svc *Service) subscribeBlockProposed(ctx context.Context, chainID *big.Int, errChan chan error) {
	sink := make(chan *wyzth_zkevml1.WYzth_zkevmL1BlockProposed)

	sub := event.ResubscribeErr(svc.subscriptionBackoff, func(ctx context.Context, err error) (event.Subscription, error) {
		if err != nil {
			log.Errorf("svc.wyzth_zkevmL1.WatchBlockProposed: %v", err)
		}
		log.Info("resubscribing to BlockProposed events")

		return svc.wyzth_zkevml1.WatchBlockProposed(&bind.WatchOpts{
			Context: ctx,
		}, sink, nil)
	})
...
func (svc *Service) subscribeBlockVerified(ctx context.Context, chainID *big.Int, errChan chan error) {
	sink := make(chan *wyzth_zkevml1.WYzth_zkevmL1BlockVerified)

	sub := event.ResubscribeErr(svc.subscriptionBackoff, func(ctx context.Context, err error) (event.Subscription, error) {
		if err != nil {
			log.Errorf("svc.wyzth_zkevmL1.WatchBlockVerified: %v", err)
		}

		log.Info("resubscribing to BlockVerified events")

		return svc.wyzth_zkevml1.WatchBlockVerified(&bind.WatchOpts{
			Context: ctx,
		}, sink, nil)
	})
