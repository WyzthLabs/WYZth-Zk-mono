...
// deployWYzth_zkevmL2 generates a L2 genesis alloc of the WYzth_zkevmL2 contract.
export async function deployWYzth_zkevmL2(
    config: Config,
    result: Result
): Promise<Result> {
...
async function generateContractConfigs(
    ...
    const contractArtifacts: any = {
        ...
        ProxiedWYzth_zkevmL2: require(path.join(
            ARTIFACTS_PATH,
            "./WYzth_zkevmL2.sol/ProxiedWYzth_zkevmL2.json"
        )),
    ...
    contractArtifacts.WYzth_zkevmL2Proxy = proxy;
    ...
    for (const [contractName, artifact] of Object.entries(contractArtifacts)) {
        ...
        switch (contractName) {
            case "ProxiedWYzth_zkevmL2":
                bytecode = linkContractLibs(
                    contractArtifacts.ProxiedWYzth_zkevmL2,
                    addressMap
                );
                break;
    ...
    return {
                ...
                addresses: {
                    [chainId]: {
                        [ethers.utils.hexlify(
                            ethers.utils.toUtf8Bytes("wyzth_zkevm")
                        )]: addressMap.WYzth_zkevmL2Proxy,
                        [ethers.utils.hexlify(
        ...
        ProxiedWYzth_zkevmL2: {
            address: addressMap.ProxiedWYzth_zkevmL2,
            deployedBytecode: linkContractLibs(
                contractArtifacts.ProxiedWYzth_zkevmL2,
                addressMap
            ),
        ...
        WYzth_zkevmL2Proxy: {
            address: addressMap.WYzth_zkevmL2Proxy,
            deployedBytecode:
                contractArtifacts.WYzth_zkevmL2Proxy.deployedBytecode.object,
            variables: {
                // WYzth_zkevmL2
            ...
            slots: {
                [ADMIN_SLOT]: contractAdmin,
                [IMPLEMENTATION_SLOT]: addressMap.ProxiedWYzth_zkevmL2,
            },
            isProxy: true,
        },
        ProxiedBridge: {
