# How WYzth_zkevm proves L2 blocks

This document specifies how the WYzth_zkevm protocol, client, and circuits work together to prove L2 blocks.
...
A valid `txList` (until [issue #13724](https://github.com/wyzth_zkevmxyz/wyzth_zkevm-mono/issues/13724) is enabled and implemented):
...
The ZKP must prove that _WYzth_zkevmL2.anchor(...)_ is the first transaction in the block, with the correct input parameters and gas limit, signed by the so called golden-touch address, and that the transaction executed successfully.

- The anchor transaction's `to` address must be the registered `wyzth_zkevm_l2` address, which is hashed into the ZKP `instance`. And the `tx.origin` must be the golden touch address.
...
- The signer must be _`WYzth_zkevmL2.GOLDEN_TOUCH_ADDRESS`_.
- The signature must use `1` as the `k` value if the calculated `r` is not `0`, other wise, `k` must be `2`. See [WYzth_zkevmL2Signer.sol](https://github.com/wyzth_zkevmxyz/wyzth_zkevm-mono/blob/main/packages/protocol/contracts/L2/WYzth_zkevmL2Signer.sol) and WYzth_zkevm [whitepaper](https://wyzth_zkevmxyz.github.io/wyzth_zkevm-mono/wyzth_zkevm-whitepaper.pdf).
...
struct BlockMetadata {
  ...
  address beneficiary;
  address treasury;
  WYzth_zkevmData.EthDeposit[] depositsProcessed;
}
```
...
In the WYzth_zkevm L2 protocol, instead of being burned, the basefee is transferred to a designated `treasury` address. To ensure the integrity of this process, the ZKP needs to verify that the treasury address specified by the WYzth_zkevm L1 contract is indeed the intended recipient.

### LibProving Verification

The actual value of the public input parameters must be consistent with the values used to hash the ZKP instance (see [**LibProving**](https://github.com/wyzth_zkevmxyz/wyzth_zkevm-mono/blob/945dabc09a668678aca1296e91d567b45ad37922/packages/protocol/contracts/L1/libs/LibProving.sol#L182)).
...
b_l1_wyzth_zkevm_addr[/wyzth_zkevmL1Address/]
