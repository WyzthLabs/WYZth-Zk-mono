...
import { WYzth_zkevmConfig } from "../contracts/L1/WYzth_zkevmConfig.sol";
import { WYzth_zkevmData } from "../contracts/L1/WYzth_zkevmData.sol";
import { WYzth_zkevmL1 } from "../contracts/L1/WYzth_zkevmL1.sol";
import { WYzth_zkevmToken } from "../contracts/L1/WYzth_zkevmToken.sol";
import { SignalService } from "../contracts/signal/SignalService.sol";
import { Strings } from "@openzeppelin/contracts/utils/Strings.sol";
import { WYzth_zkevmL1TestBase } from "./WYzth_zkevmL1TestBase.t.sol";

contract WYzth_zkevmL1_NoCooldown is WYzth_zkevmL1 {
    function getConfig()
        public
        pure
        override
    ...
    {
        config = WYzth_zkevmConfig.getConfig();
...
contract Verifier {
    fallback(bytes calldata) external returns (bytes memory) {
        return bytes.concat(keccak256("wyzth_zkevm"));
    }
}

contract WYzth_zkevmL1Test is WYzth_zkevmL1TestBase {
    function deployWYzth_zkevmL1() internal override returns (WYzth_zkevmL1 wyzth_zkevmL1) {
        wyzth_zkevmL1 = new WYzth_zkevmL1_NoCooldown();
    }

    function setUp() public override {
        WYzth_zkevmL1TestBase.setUp();
    ...
    function test_more_blocks_than_ring_buffer_size() external {
        depositWYzth_zkevmToken(Alice, 1e6 * 1e8, 100 ether);
        depositWYzth_zkevmToken(Bob, 1e6 * 1e8, 100 ether);
        depositWYzth_zkevmToken(Carol, 1e6 * 1e8, 100 ether);
        ...
        for (
            ...
            WYzth_zkevmData.BlockMetadata memory meta =
    ...
    function test_multiple_blocks_in_one_L1_block() external {
        depositWYzth_zkevmToken(Alice, 1000 * 1e8, 1000 ether);
    ...
    function test_verifying_multiple_blocks_once() external {
        depositWYzth_zkevmToken(Alice, 1e6 * 1e8, 1000 ether);
    ...
    function testEthDepositsToL2Reverts() external {
        ...
        depositWYzth_zkevmToken(Alice, 0, maxAmount + 1 ether);
    ...
    function testEthDepositsToL2Gas() external {
        ...
        depositWYzth_zkevmToken(Alice, 1e6 * 1e8, 100_000 ether);
    ...
    function test_deposit_hash_creation() external {
        ...
        depositWYzth_zkevmToken(Alice, 1e6 * 1e8, maxAmount + 1 ether);
        depositWYzth_zkevmToken(Bob, 0, maxAmount + 1 ether);
        depositWYzth_zkevmToken(Carol, 0, maxAmount + 1 ether);
        depositWYzth_zkevmToken(Dave, 0, maxAmount + 1 ether);
        depositWYzth_zkevmToken(Eve, 0, maxAmount + 1 ether);
        depositWYzth_zkevmToken(Frank, 0, maxAmount + 1 ether);
        depositWYzth_zkevmToken(George, 0, maxAmount + 1 ether);
        depositWYzth_zkevmToken(Hilbert, 0, maxAmount + 1 ether);
