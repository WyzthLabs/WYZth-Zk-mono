...
library LibEthDepositing {
    ...
    event EthDeposited(WYzth_zkevmData.EthDeposit deposit);
    ...
    function processDeposits(
        ...
        internal
        returns (WYzth_zkevmData.EthDeposit[] memory deposits)
    {
        uint256 numPending =
            state.numEthDeposits - state.nextEthDepositToProcess;
        if (numPending < config.ethDepositMinCountPerBlock) {
            deposits = new WYzth_zkevmData.EthDeposit[](0);
        } else {
            deposits = new WYzth_zkevmData.EthDeposit[](
            ...
            for (uint256 i; i < deposits.length;) {
                ...
                deposits[i] = WYzth_zkevmData.EthDeposit({
    ...
    function hashEthDeposits(WYzth_zkevmData.EthDeposit[] memory deposits)
        internal
        pure
        returns (bytes32)
    {
