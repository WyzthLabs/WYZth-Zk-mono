...
library LibVerifying {
    ...
    function init(
        WYzth_zkevmData.State storage state,
        ...
        internal
    {
        ...
        WYzth_zkevmData.Block storage blk = state.blocks[0];
        blk.proposedAt = timeNow;
        blk.nextForkChoiceId = 2;
        blk.verifiedForkChoiceId = 1;

        WYzth_zkevmData.ForkChoice storage fc = state.blocks[0].forkChoices[1];
        ...
        internal
    {
        ...
        WYzth_zkevmData.Block storage blk = state.blocks[i % config.ringBufferSize];
        ...
        while (i < state.numBlocks && processed < maxBlocks) {
            ...
            WYzth_zkevmData.ForkChoice storage fc = blk.forkChoices[fcId];
        ...
        if (processed > 0) {
            ...
            if (config.relaySignalRoot) {
                ...
                // WYzth_zkevmL1 deployments, if they share the same signal service,
                // can relay the signal to their corresponding WYzth_zkevmL2 contract.
    ...
    function _markBlockVerified(
        WYzth_zkevmData.State storage state,
        WYzth_zkevmData.Block storage blk,
        WYzth_zkevmData.ForkChoice storage fc,
        uint24 fcId,
