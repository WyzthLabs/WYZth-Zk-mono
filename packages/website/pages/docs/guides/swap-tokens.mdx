...
This guide will help you interact with Swap, which is a fork of Uniswap v2 that WYzth_zkevm has deployed only for testing purposes.

## Prerequisites

- You must have some tokens (at least ETH) on WYzth_zkevm (Alpha-3 testnet). You can [receive some ETH, HORSE, and BLL from the faucets](/docs/guides/receive-tokens) and then [bridge them to WYzth_zkevm](/docs/guides/use-the-bridge).

## Steps

<Steps>
    ### Navigate to Swap
    Navigate to [https://swap.test.wyzth_zkevm.xyz](https://swap.test.wyzth_zkevm.xyz).

    ### Connect to the WYzth_zkevm testnet
