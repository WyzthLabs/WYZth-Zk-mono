...
# Run a WYzth_zkevm node

## Overview

This guide will walk you through the process of operating a WYzth_zkevm node via [simple-wyzth_zkevm-node](https://github.com/wyzth_zkevmxyz/simple-wyzth_zkevm-node). You will be able to:

- Run a WYzth_zkevm node easily from the command line on Windows, Mac, and Linux.
...
- Consult the [Geth minimum hardware requirements](https://github.com/ethereum/go-ethereum#hardware-requirements), but ignore the storage requirement as WYzth_zkevm nodes will require less storage. Around 50GB should be more than enough initially, but over time it could become insufficient as the chain grows. As of `2023-06-21T16:59:11+00:00` a WYzth_zkevm node sync takes **17.3 GB**.

## Steps

<Steps>
### Clone simple-wyzth_zkevm-node
...
<Tabs items={["Mac/Linux", "Windows"]} defaultIndex="0">
  <Tab>
  ```sh
  git clone https://github.com/wyzth_zkevmxyz/simple-wyzth_zkevm-node.git
  cd simple-wyzth_zkevm-node
  ```
  </Tab>
  <Tab>
  ```sh
...
<Tabs items={["Local", "Alchemy", "Infura"]} defaultIndex="0">
  <Tab>
  If you are running a local Sepolia node, you cannot reference the L1 endpoints as `http://127.0.0.1:8545` and `ws://127.0.0.1:8546` because that is local to inside the simple-wyzth_zkevm-node Docker image. Depending on your firewall setup, you can do a few things. You can try:
  ...
  - for "name", enter "WYzth_zkevm A3 Node", or your preferred name. 
...
<Callout>
  If you ran a testnet node previously make sure to first run a `docker compose down -v` to remove the old volumes. Also, it may take some time to synchronize from the genesis block. You can monitor this progress through logs, or in the local grafana dashboard and see the latest L2 chain status in the [WYzth_zkevm block explorer](https://explorer.test.wyzth_zkevm.xyz/).
</Callout>
...
Update the `simple-wyzth_zkevm-node` Docker images:
...
```sh
docker compose logs -f wyzth_zkevm_client_proposer
```

#### View the prover image's logs

```sh
docker compose logs -f wyzth_zkevm_client_prover_relayer
```
...
This error just means that the port wyzth_zkevm-node is trying to allocate is already being used by either some other node you're running
or by a left-over container of wyzth_zkevm node if you did not close down the node correctly, you can try running `docker compose down` in the simple-wyzth_zkevm-node folder and start it again using `docker compose up -d`
...
You are in the wrong directory, `cd simple-wyzth_zkevm-node` and run the command again.
...
#### `wyzth_zkevm_client_prover_relayer not printing any logs`
...
#### `simple-wyzth_zkevm-node-wyzth_zkevm_client_prover_relayer-1  | /bin/sh: /script/start-prover-relayer.sh: not found`
